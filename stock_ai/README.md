...
### ⑤ 過熱・連続上昇ペナルティ（最大 -23 点）

ランキング検証（後述）で「当日の前日比が大きい銘柄ほど翌日は下落しやすい」傾向が
確認されたため、過熱した銘柄ほど減点して的中率向上を図る調整項目（`config.py`で調整可能）。
...
⑤⑥は2026年6月22日の1日分のデータを基に追加したが、複数日（6日・約180件）で
再検証した結果、効果は確認できていない（「ランキング検証」セクション参照）。
